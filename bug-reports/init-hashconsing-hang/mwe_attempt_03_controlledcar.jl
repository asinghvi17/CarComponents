# Experiment 03 (CONTROL): the known-hanging ControlledFlatRoadCar, exact repro recipe
# from solve_and_save.jl, but STOP after ODEProblem (no solve/serialize).
# Purpose: confirm THIS machine still reproduces the 7.5hr hang today. timeout 150 => reproduced.
T0 = time()
...
defs = build_defs()
logmsg("defs built ($(length(defs)) pairs). Building ODEProblem (optimize=:basic)... [KNOWN 7.5hr HANG STEP]")
tp = time()
